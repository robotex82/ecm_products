...
feature 'Ecm::Products::ProductPicture admin' do
  ...
  def set_resource_path
    @resource_path = "ecm_products_product_pictures"
  end

  def set_resource_class
    @resource_class = Ecm::Products::ProductPicture
  end
  ...
  def fill_new_form
    select @product.name, :from => 'ecm_products_product_picture[ecm_products_product_id]'
    fill_in "ecm_products_product_picture[name]", :with => "Example product picture"
    attach_file "ecm_products_product_picture[image]", File.join(Rails.root, "spec/files", "ecm/products", "product_picture/image.jpg")
    select 'textile', :from => 'ecm_products_product_picture[markup_language]'
  end

  def fill_edit_form
    fill_in "ecm_products_product_picture[name]", :with => "An updated product picture"
  end
